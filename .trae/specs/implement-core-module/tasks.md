...
- [x] Task 4: 实现 core::params —— 参数类型
  - 创建 `src/core/params.rs`
  - 定义 `GridSize` 结构体（rows/usize + cols/usize），派生 Debug/Clone/Copy/PartialEq/Serialize/Deserialize
  - 定义 `OutputFormat` 枚举（Parquet/Text/Binary），派生 Debug/Clone/Copy/Default/PartialEq/Eq/Serialize/Deserialize
...
- [x] Task 5: 实现 core::generator —— Generator trait
...
- [x] Task 6: 实现 core::registry —— GeneratorRegistry
...
- [x] Task 8: 运行全量测试并验证
  - 运行 `cargo test` 确保所有单元测试通过（**36 passed, 0 failed**）
  - 运行 `cargo clippy` 确保无实质性警告（仅 dead_code，符合预期）
