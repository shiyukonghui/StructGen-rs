...
### 1. OutputFormat 新增变体 (`src/core/params.rs`)
...
### 2. SinkAdapterFactory 类型变更 (`src/sink/factory.rs`)
...
### 3. create_adapter 函数修改 (`src/main.rs`)
...
fn create_adapter(format: OutputFormat, config: &Value) -> CoreResult<Box<dyn SinkAdapter>> {
    match format {
        ...
        OutputFormat::NpyBatch => {
            let npy_config: NpyBatchConfig = if config.is_null() {
                return Err(CoreError::InvalidParams(
                    "NpyBatch 输出格式需要在 extensions.sink_config 中提供配置".into(),
                ));
            } else {
                serde_json::from_value(config.clone()).map_err(|e| {
                    CoreError::SerializationError(format!("NpyBatchConfig 解析失败: {}", e))
                })?
            };
...
### 4. executor.rs 提取 sink_config

```rust
// execute_shard_inner 中修改
